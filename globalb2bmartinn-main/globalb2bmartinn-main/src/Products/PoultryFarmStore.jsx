...
const PoultryFarmStore = () => {
  ...
  const [filteredData, setFilteredData] = useState(poultryFarmData);
  const [searchContext, setSearchContext] = useState("Premium Poultry Suppliers");
  ...
  useEffect(() => {
    const fetchPoultryProducts = async () => {
      try {
        const res = await axios.get(`${apiEndpoint}/products/category/Poultry`);
        setDbProducts(res.data);
        setFilteredData([...poultryFarmData, ...res.data]);
      } catch (err) {
        console.error("Failed to fetch poultry products:", err);
        setFilteredData(poultryFarmData);
      }
    };
  ...
  const handleApplyFilters = () => {
    let result = [...poultryFarmData, ...dbProducts];
    ...
    setSearchContext(selectedTypes.length > 0 ? selectedTypes.join(", ") + " Poultry Suppliers" : "Premium Poultry Suppliers");
  };

  const resetFilters = () => {
    setSelectedTypes([]);
    setLocationQuery("");
    setFilteredData([...poultryFarmData, ...dbProducts]);
    setSearchContext("Premium Poultry Suppliers");
    setActiveChip("All");
  };
  ...
  const handleCategoryChip = (cat) => {
    ...
    const allData = [...poultryFarmData, ...dbProducts];
    ...
    setFilteredData(result);
    setSearchContext(cat + " Poultry");
  };

  const categories = [
    { label: "All",     img: "/assets/chi1.jpg" },
    { label: "Chicken", img: "/assets/chi2.jpg" },
    { label: "Eggs",    img: "/assets/egg1.jpg" },
    { label: "Feed",    img: "/assets/chi3.jpg" },
    { label: "Quail",   img: "/assets/egg1.jpg" },
  ];

  return (
    <div className="marketplace-container">
      <Helmet>
        <title>Verified Poultry Farms & Egg Suppliers | Global B2B Mart</title>
        <meta name="description" content="Find top poultry farms. Connect with verified suppliers of chicken, eggs, and poultry feed. Premium B2B agricultural marketplace." />
      </Helmet>
          ...
          <div className="filter-group-container">
            <div className="filter-group">
              ...
              <div className="checkbox-group">
                {["Chicken", "Eggs", "Poultry Feed", "Quail Eggs"].map(type => (
            ...
            {categories.map((cat) => (
              ...
              >
                <div className="chip-img-ring">
                  <img src={cat.img} alt={cat.label} onError={(e) => e.target.src="/assets/chi1.jpg"} />
                </div>
              ...
              {filteredData.map((item, index) => (
                <div className="product-card" key={index}>
                  <div className="card-image-wrapper">
                    <img 
                      src={item.imgSrc || (item.images?.[0] ? `${apiEndpoint}${item.images[0].replace(/\\/g, '/')}` : "/assets/chi1.jpg")} 
