import React, { useEffect, useRef } from "react";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
import "./category_premium.css";

// Register ScrollTrigger
gsap.registerPlugin(ScrollTrigger);

const ToolsAndEquipment = () => {
    const mainRef = useRef(null);

    useEffect(() => {
        const timer = setTimeout(() => {
            ScrollTrigger.refresh();
        }, 100);

        const ctx = gsap.context(() => {
            gsap.fromTo(".pagetitle h1", 
                { y: 30, opacity: 0 },
                { y: 0, opacity: 1, duration: 1.2, ease: "power4.out" }
            );

            const sections = gsap.utils.toArray(".category-section");
            sections.forEach((section) => {
                gsap.fromTo(section, 
                    { y: 50, opacity: 0 },
                    {
                        y: 0,
                        opacity: 1,
                        duration: 1,
                        scrollTrigger: {
                            trigger: section,
                            start: "top 90%",
                            toggleActions: "play none none none"
                        }
                    }
                );
            });

            gsap.fromTo(".l1-box", 
                { scale: 0.95, opacity: 0 },
                {
                    scale: 1,
                    opacity: 1,
                    duration: 0.8,
                    stagger: 0.1,
                    ease: "power2.out",
                    delay: 0.3
                }
            );
        }, mainRef);

        return () => {
            clearTimeout(timer);
            ctx.revert();
        };
    }, []);

    const products = [
        {
            id: "hand-tools",
            title: 'Hand Tools',
            imageSrc: '/assets/eqip1.jpg',
            links: [
                { name: 'Hammers', url: '#' },
                { name: 'Screwdrivers', url: '#' },
                { name: 'Pliers', url: '#' },
                { name: 'Wrenches', url: '#' },
            ],
        },
        {
            id: "power-tools",
            title: 'Power Tools',
            imageSrc: '/assets/eqip2.jpg',
            links: [
                { name: 'Drills', url: '#' },
                { name: 'Grinders', url: '#' },
                { name: 'Saws', url: '#' },
                { name: 'Sanders', url: '#' },
            ],
        },
        {
            id: "construction",
            title: 'Construction Tools',
            imageSrc: '/assets/eqip7.avif',
            links: [
                { name: 'Concrete Mixers', url: '#' },
                { name: 'Masonry Tools', url: '#' },
                { name: 'Tile Cutters', url: '#' },
                { name: 'Trowels', url: '#' },
            ],
        },
        {
            id: "measuring",
            title: 'Measuring Tools',
            imageSrc: '/assets/eqip5.avif',
            links: [
                { name: 'Tape Measures', url: '#' },
                { name: 'Calipers', url: '#' },
                { name: 'Levels', url: '#' },
                { name: 'Laser Measures', url: '#' },
            ],
        },
        {
            id: "welding",
            title: 'Welding Tools',
            imageSrc: '/assets/eqip8.webp',
            links: [
                { name: 'Welding Machines', url: '#' },
                { name: 'Welding Helmets', url: '#' },
                { name: 'Welding Rods', url: '#' },
                { name: 'Welding Gloves', url: '#' },
            ],
        },
        {
            id: "plumbing",
            title: 'Plumbing Tools',
            imageSrc: '/assets/eqip9.jpg',
            links: [
                { name: 'Pipe Wrenches', url: '#' },
                { name: 'Pipe Cutters', url: '#' },
                { name: 'Plungers', url: '#' },
                { name: 'Pipe Benders', url: '#' },
            ],
        },
    ];

    return (
        <div ref={mainRef}>
            <div className="pagetitle">
                <h1>Tools & Equipment Trade Directory & Networking Platform</h1>
            </div>

            <nav className="category-nav-bar">
                <div className="category-nav-pill">
                    {products.map((p) => (
                        <a key={p.id} href={`#${p.id}`}>{p.title.split(' ')[0]}</a>
                    ))}
                </div>
            </nav>

            <div className="l1-section">
                <div className="container">
                    <section className="category-section">
                        <div className="row l1-box-wrap">
                            {products.map((product, index) => (
                                <div key={index} id={product.id} className="col-4">
                                    <div className="l1-box">
                                        <h2 className="l1-heading">
                                            <a href="#">{product.title}</a>
                                        </h2>
                                        <div className="l1-content-flex">
                                            <div className="l1-image-area">
                                                <figure className="glass-image-container">
                                                    <img
                                                        src={product.imageSrc}
                                                        alt={product.title}
                                                        width={95}
                                                        height={95}
                                                        loading="lazy"
                                                    />
                                                </figure>
                                            </div>
                                            <div className="l1-links-area">
                                                <ul>
                                                    {product.links.map((link, idx) => (
                                                        <li key={idx}>
                                                            <a href={link.url}>{link.name}</a>
                                                        </li>
                                                    ))}
                                                    <li className="view-more-li">
                                                        <a href="#">View More</a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ))}
                        </div>
                    </section>
                </div>
            </div>

            <div className="content-page-premium">
                <div className="container">
                    <div className="glass-info-card">
                        <p>
                            GlobalB2BMart is your exhaustive resource for industrial tools and construction equipment. We bridge professionals with high-grade machinery manufacturers, offering precision measuring tools, heavy-duty welding gear, and essential construction apparatus.
                        </p>
                    </div>
                </div>
            </div>
        </div>
    );
}

export default ToolsAndEquipment;
